...
res_file <- "./results_sim_new/c1_V0.5_rep4.rds"
...
PR_data$PR_data[method=="lfmm_q"][which.max(PR),paste(paste0("TP=",TP),paste0("FP=",FP),paste0("FN=",FN),paste0("PR=",round(PR,2)),sep=" | ")]
PR_data$PR_data_C[method=="C_joint"][which.max(PR),paste(paste0("TP=",TP),paste0("FP=",FP),paste0("FN=",FN),paste0("PR=",round(PR,2)),sep=" | ")]

tau_C_joint<- PR_data$PR_data_C[method=="C_joint"][which.max(PR),tau_C]
alpha_lfmm <- PR_data$PR_data[method=="lfmm_q"][which.max(PR),alpha]


## get true and false posities for joint analyses

ORs_max_PR <- unlist(PR_data$PR_data_C[method=="C_joint"][which.max(PR),ORs],recursive = FALSE)
...
ORs_max_PR <- unlist(PR_data$PR_data[method=="lfmm_q"][which.max(PR),ORs],recursive = FALSE)
ORs_max_PR <- data.table(unlist(ORs_max_PR),rep(names(ORs_max_PR),sapply(ORs_max_PR,length)))
focals_max_PR <- PR_data$PR_data[method=="lfmm_q"][which.max(PR),OR_focals][[1]]
...
SNP_res[,cor(max_LD_with_QTN,ldw_best)^2]


plot_data_manh <- prep_manhattan(SNP_res[,.(bp=Pos,Chr,marker,C=C_joint,C_lfmm_F_prime,C_emx_F_prime,C_emx_q,C_lfmm_q,ld_w=ldw_best,focal_QTN,EMX=-log10(emx_q),LFMM=-log10(lfmm_q),test_joint,OR_joint,OR_lfmm,test_lfmm,type,max_LD_with_QTN,p_Va)],chr_cols = c("white","grey80"),spacer =0)

plot_manhattan_gg(plot_data_manh)
