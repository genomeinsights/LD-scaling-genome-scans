...
## performance statistics for best
...
focals_max_PR <- PR_data$PR_data_C[method=="C_joint"][which.max(PR),OR_focals][[1]]

SNP_res[,OR_joint := ORs_max_PR$V2[match(marker, ORs_max_PR$V1)]]
SNP_res[!is.na(OR_joint) & focal_QTN %in% focals_max_PR,test_joint := "True positive"]
SNP_res[!is.na(OR_joint) & !focal_QTN %in% focals_max_PR,test_joint := "False positive"]
...
ld_w_draws <- get_ld_w_draws(gds,decay_tbl = decay_tbl_final,slide_win_ld = 1000,n_draws = 1,min = 0.9,max = 0.91,n_cores = 8)
ld_w <- rbindlist(ld_w_draws[draw==1,ld_w])
SNP_res[,ld_w := ld_w[match(ids$snp_id,SNP1),median_r2]]

##
SNP_res[,cor(max_LD_with_QTN,C_joint)^2]
SNP_res[,cor(max_LD_with_QTN,-log10(lfmm_q))^2]
SNP_res[,cor(max_LD_with_QTN,ld_w,use = "pair")^2]


plot_data_manh <- plot_manhattan(SNP_res[,.(bp=Pos,Chr,marker,C=C_joint,C_lfmm_F_prime,C_emx_F_prime,C_emx_F,C_lfmm_F,ld_w,focal_QTN,EMX=-log10(emx_q),LFMM=-log10(lfmm_q),test_joint,OR_joint,OR_lfmm,test_lfmm,type,max_LD_with_QTN,p_Va)],chr_cols = c("white","grey80"),spacer =0)

p1 <-  ggplot(plot_data_manh$don, aes(BPcum,C,col=OR_joint,shape=type)) +
  geom_rect(data=plot_data_manh$rect_data, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=Inf),fill=plot_data_manh$rect_data$col,alpha=0.5, linewidth = 0.25,inherit.aes = FALSE) +
  geom_point(data=plot_data_manh$don,size=1,alpha=1) +
  geom_point(data=plot_data_manh$don[type=="QTN" & !is.na(OR_joint)],size=3,alpha=1,shape=3) +
  ...
  scale_x_continuous(label = plot_data_manh$axisdf$Chr, breaks= plot_data_manh$axisdf$center ) +
  theme(
    aspect.ratio = 0.25,
...
p2 <-  ggplot(plot_data_manh$don, aes(BPcum,LFMM,col=OR_joint,shape=type)) +
  geom_rect(data=plot_data_manh$rect_data, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=Inf),fill=plot_data_manh$rect_data$col,alpha=0.5, linewidth = 0.25,inherit.aes = FALSE) +
  geom_point(data=plot_data_manh$don[],size=1,alpha=1) +
  geom_point(data=plot_data_manh$don[LFMM<1.32],size=1,alpha=1,aes(col=OR_lfmm)) +
  geom_point(data=plot_data_manh$don[type=="QTN" & !is.na(OR_lfmm)],size=3,alpha=1,shape=3) +
  scale_shape_manual(values = c(20,3),name=NULL)+
  scale_color_manual(values =rep(col_vector,3),guide="none")+
  theme_bw() +
  guides(shape = guide_legend(override.aes = list(size = 3)))+
...
p3 <-  ggplot(plot_data_manh$don, aes(BPcum,C,col=max_LD_with_QTN)) +
  geom_rect(data=plot_data_manh$rect_data, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=Inf),fill=plot_data_manh$rect_data$col,alpha=0.5, linewidth = 0.25,inherit.aes = FALSE) +
  geom_point(data=plot_data_manh$don,size=1,alpha=1) +
  geom_point(data=plot_data_manh$don[type=="QTN" & !is.na(OR_joint)],size=3,alpha=1,shape=3) +
...
p4 <-  ggplot(plot_data_manh$don, aes(BPcum,ld_w,col=max_LD_with_QTN)) +
  geom_rect(data=plot_data_manh$rect_data, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=Inf),fill=plot_data_manh$rect_data$col,alpha=0.5, linewidth = 0.25,inherit.aes = FALSE) +
  geom_point(data=plot_data_manh$don,size=1,alpha=0.5) +
  geom_point(data=plot_data_manh$don[type=="QTN" & !is.na(OR_joint)],size=3,alpha=1,shape=3) +
  scale_shape_manual(values = c(2,20,3))+
...
## get AUC data
AUC_lfmm <- data_AUC[c==SNP_res[1,c] & V==SNP_res[1,V] & rep==SNP_res[1,rep]  & method=="lfmm_F",round(AUC_norm,2)]
