get_bg_ld <- function(gds, n_sub = 5000, q = 0.95, seed = NULL) {
  
  if (!is.null(seed)) set.seed(seed)
  
  ids <- read_gds_ids(gds)
  n_snps <- length(ids$snp_id)
  
  if (n_snps < 2L) stop("Not enough SNPs to estimate background LD.")
  
  ## sample SNPs, but ensure multiple chromosomes
  snp_pool <- unlist(lapply(split(seq_len(n_snps), ids$snp_chr), function(ix) {
    sample(ix, min(length(ix), ceiling(n_sub / length(unique(ids$snp_chr)))))
  }))
